...
static DIMOUSESTATE2   lastMouseState;
//--------------------------------- game pad
...
bool GetKeyboardAnyTrigger()
{
	return g_keyAnyTrigger;
}
...
BOOL IsMouseLeftHeld(void)
{
	return (BOOL)(lastMouseState.rgbButtons[0] & 0x80) && (mouseState.rgbButtons[0] & 0x80);	// 前stateと現state両方がオンかをチェック
}
BOOL IsMouseLeftReleased(void)
{
	return (BOOL)(lastMouseState.rgbButtons[0] & 0x80) && !(mouseState.rgbButtons[0] & 0x80);	// 前stateがオンで現stateがオフかをチェック
}
...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
        ...
        sticks.x = (float)dijs.lX / 32768.0f;
        sticks.y = (float)dijs.lY / 32768.0f;
        float tempX = (dijs.lRx );
        float tempY = (dijs.lRy );
        float tempX_0 = ( tempX - 32768.0f );
        float tempY_0 = ( tempY - 32768.0f );
        sticks.z = tempX_0 / 32768.0f;
        sticks.w = tempY_0 / 32768.0f;
